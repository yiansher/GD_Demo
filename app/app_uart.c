...
struct rt_ringbuffer *g_rb;
rt_uint8_t *g_pool;
//uint8_t u8RxData[1] = {0};
uint8_t u8RxData = 0;
uint8_t *u8RxData_p = &u8RxData;
...
//    RT_ASSERT(size > 0);
    ...
    APP_TRACE("ringbuffer create success");
    return g_rb;
exit:
    APP_TRACE("ringbuffer create fail");
    return g_rb;
}

void app_ringbuffer_init(rt_uint32_t size)
{
    g_rb = rt_ringbuffer_create(size);
}
...
static void uart_sendbyte(uint8_t *byte, uint16_t size)
{
    uint16_t i = 0;
    uint8_t *byte_p = byte;
    for(i=0;i<size;i++)
    {
        usart_data_transmit(UART_COM, (uint8_t)*byte_p);
...
void app_uart_rxCallback(void)
{
    if( RESET != usart_interrupt_flag_get( UART_COM, USART_INT_FLAG_RBNE ) )
    {
        /* receive data */
        *u8RxData_p = usart_data_receive( UART_COM );
        rt_ringbuffer_putchar(g_rb, *u8RxData_p);
